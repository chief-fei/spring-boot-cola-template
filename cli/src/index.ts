...
async function main() {
  ...
  const s = p.spinner();
  s.start("加载依赖信息...");
  const [initializrData, { meta }] = await Promise.all([
    fetchInitializrData(),
    downloadTemplate(),
  ]);
  s.stop("依赖信息加载完成");

  const config: ProjectConfig = await promptProjectConfig(initializrData, meta);

  const projectDir = await generate(config, meta);
